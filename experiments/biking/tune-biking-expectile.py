...
q = float(sys.argv[2])
mydata = ArffToPytorch(sys.argv[1], target='count')
    ...
    for initlr, tzero, eta, gammamin, gammamax, nalgos in (
       ...
       for _ in range(59)
        ...
        learnOnline(mydata, q=q, seed=4545, initlr=initlr, tzero=tzero, batch_size=8, 
                    ...
                    modelfactory=lambda x: CauchyTruncatedNormal(dobs=x.shape[1], numrff=1024, sigma=1/10, device='cpu', cost=cost, price=price, approxargmax=approxargmax))
flass()
